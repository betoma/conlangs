...
This syllable structure can also be expressed in Recursive Baerian Phonotactic Notation\footnote{insert link to the github when you've got internet sparky} as follows:
$$
[_{\sigma}\left[_{\omega}\begin{matrix} \begin{matrix}\textrm{S}\\\nm\end{matrix} \cdot \textrm{C} \cdot \begin{matrix}\textrm{R}\\\nm\end{matrix}\\\nm\end{matrix}\right]\left[_{\nu}\begin{matrix}\textrm{V}\cdot\begin{matrix}\textrm{R}\\\nm\end{matrix}\\\textrm{R}\end{matrix}\right]\left[_{\kappa}\begin{matrix}\textrm{C}\\\nm\end{matrix}\right]]
$$
...
Among those who don't subscribe to an analysis of Geobo\engma{} as possessing vowelless syllables, the dominant approach is to analyze the syllable structure as underlyingly something fairly simple and regular, such as that shown in Figure \ref{fig:sylb-struc-schwa}, with 
